...
(defn plot-data
  [n x-grid-step buffer width height]
  {:x-axis (viz/linear-axis
            {:domain      [0 n]
             :range       [50 (- width 10)]
             :major       (or x-grid-step (Math.ceil (* n 0.2)))
             :minor       (or (/ x-grid-step 5) (Math.ceil (* n 0.1)))
             :pos         (- height 50)})
...
(defn plot-n
  [n x-grid-step buffer width height]
  (->> (plot-data n x-grid-step buffer width height)
       (viz/svg-plot2d-cartesian)
       (svg/svg {:width width :height height})))

(defn plot-buffers
  [n x-grid-step & buffers]
       ...
       [:tr
        (doall
         (for [[label buffer] (partition 2 2 buffers)]
           [:td (plot-n n x-grid-step buffer
